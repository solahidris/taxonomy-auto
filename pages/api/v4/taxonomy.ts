...
type V4Evaluation = {
  v3_metrics: { name: string; total_niches: number; llm_sub_niches: number; niches_processed: number }
  v4_metrics: {
    name: string
    total_niches: number
    main_niches: number
...
let evaluationCache: V4Evaluation | null = null
...
function loadEvaluation(): V4Evaluation | null {
  if (evaluationCache) return evaluationCache
  const fp = path.join(process.cwd(), 'data', 'v4', 'evaluation.json')
  if (!fs.existsSync(fp)) return null
